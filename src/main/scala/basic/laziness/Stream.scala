...
trait Stream[+A] {
  ...
  def toList: List[A] = {
    ...
    @annotation.tailrec
    def go(xs:Stream[A],z:B)(f: (A,B) => B): B = xs match {
      case Empty => z
  ...
  def takeViaUnfold(n: Int): Stream[A] = unfold((this,n)) {
    case (Cons(h,t), i) if(i > 0)  => Some( (h() , ( cons(h(),t()), n-1) ))
    case (Cons(h,t), i) if(i == 0) => Some( (h() , ( empty, n-1) ))
    case _ => None
  }
  ...
  def subSequenceViaTails[B >: A](sx: Stream[B]): Boolean = 
    tails exists(a => a startsWith sx)
...
object Stream {
  ...
  def cons[A](hd: => A, tl: => Stream[A]): Stream[A] = {
    lazy val head = hd // lazy 로 memozation 을 하여 최초 평가시 1번만 평가되어 재사용 된다.
    lazy val tail = tl
    Cons(() => head, () => tail)
  }
  ...
  def apply[A](xs: A *): Stream[A] = //A * 은 Seq type 이다. 
