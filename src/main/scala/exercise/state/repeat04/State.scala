package exercise.state.repeat04

import exercise.state.repeat04.State._
...
object State {
  ...
  def sequenceExplicit[S,A](xs: List[State[S,A]]): State[S,List[A]] = {
    ...
    def go(s: S, xs: List[State[S,A]], rs: List[A]): (List[A], S) = xs match {
      case h::t => h run s match { case (a,s2) => go(s2, t, a::rs ) }
  ...
  def sequence[S,A](xs: List[State[S,A]]): State[S,List[A]] = {
    ...
    xs.foldLeft[B](s => s)((b,a) => x => b(a.map2(x)((a1,x1) => a1::x1)) )(unit(Nil))
  }
...
case class Machine(locked: Boolean, cnadies: Int, coins: Int)
...
object CandySlotMachine {
  ...
  def simulatedMachine(inputs: List[Input]): State[Machine,(Int,Int)] = 
    sequence(inputs.map(modify[Machine] _ compose update)).flatMap(u => get.map(m => (m.cnadies,m.coins)));
    
  //ex-16) simulatedMachine을 for 표현식으로 구현하라.
  def simulatedMachineByFor(inputs: List[Input]): State[Machine,(Int,Int)] = for {
    u  <- sequence(inputs.map(modify[Machine] _ compose update))
    s  <- get 
  }yield((s.cnadies,s.coins))
...
object StateDriver extends App {
  ...
  import exercise.state.repeat04.CandySlotMachine._
