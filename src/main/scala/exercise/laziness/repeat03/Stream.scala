package exercise.laziness.repeat03

import exercise.laziness.repeat03.Stream._
...
trait Stream[+A] {
  ...
  def toListNotReverse: List[A] = {
    ...
    go(this,(y:List[A]) => y)((a,f) => x => f(a::x))(Nil) 
  }
  ...
  def takeViaUnfold(n: Int): Stream[A] = unfold((this,n)) {
    case (Cons(h,t),n1) if n1 > 0 => Some((h(),(t(),n-1)))
    case (_,_) => None
  }
  
  //ex-28)unfold를 이용하여 takeWhile를 구현하라.
  def takeWhileViaUnfold(p: A => Boolean): Stream[A] = unfold(empty[A]){
  ...
  def subSequence[B >: A](sub: Stream[B]): Boolean =
    sub.foldRight(true)((a,b) => this.exists(c => a == c) && b)
  ...
  def startsWith[B >: A](sub: Stream[B]): Boolean =
    sub zipAll this takeWhile(t => !t._1.isEmpty) forAll(t => t._1 == t._2)
  ...
  def subSequenceViaTails[B >: A](sx: Stream[B]): Boolean = 
    tails exists(ma => ma startsWith sx)
...
object Stream {
  ...
  def formViaUnfold(n: Int): Stream[Int] =
...
//  println(fibsViaUnfold.takeOrigin(8).toList)
  ...
  println(xs01.zipWith(xs02)((a,b) => a + b).toList)
}