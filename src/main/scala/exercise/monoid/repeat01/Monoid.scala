package exercise.monoid.repeat01
...
object Monoid {
  ...
  def endoMonoid[A]: Monoid[A => A] = new Monoid[A => A] {
    def zero: A => A = a => a
    def op(af: A => A, bf: A => A): A => A = af compose bf
  }
  ...
  //ex-10) List에 대한 foldRight를 listFoldMap 함수를 이용하여 구현하라
  def listFoldMap[A,B](xs: List[A])(f: A => B)(m: Monoid[B]): B =
    xs.foldRight[B](m.zero)((a,b) => m.op(f(a), b))
    
    
  //ex-11) List에 대한 listFoldMap 함수를 foldRight를 이용하여 구현하라
  ...
  def par[A](m: Monoid[A]): Monoid[Par[A]] = new Monoid[Par[A]] {
    ...
    def op(pa: Par[A], pb: Par[A]): Par[A] = pa.map2(pb)((a,b) => m.op(a, b))
  }
    ...
    val m: Monoid[Option[(Int,Int,Boolean)]] = new Monoid[Option[(Int,Int,Boolean)]] {
      ...
      def op(o1 : Option[(Int,Int,Boolean)], o2: Option[(Int,Int,Boolean)]) = (o1,o2) match {
        ...
        case (Some((x1,y1,t1)), Some((x2,y2,t2))) => 
          Some(x1 min x2, y1 max y2, t1 && t2 && y1 <= x2)
      }
    }
  ...
  def mergeMapMonoid[K,V](m: Monoid[V]): Monoid[Map[K,V]] = new Monoid[Map[K,V]] {
    ...
    def op(m1: Map[K,V], m2: Map[K,V]): Map[K,V] = {
      (m1.keySet ++ m2.keySet).foldLeft[Map[K,V]](Map.empty)((ms,k) => 
        ms.updated(k, m.op(m1.getOrElse(k, m.zero),m2.getOrElse(k, m.zero))))
    }
  }
  ...
  def bag[A](xs: IndexedSeq[A]): Map[A,Int] = 
    foldMapV[A,Map[A,Int]](xs,mergeMapMonoid(intAddMonoid))(a => Map(a -> 1))
    
}

//ex-16)여러 자료구조에서 fold의 공통점을 뽑아 Foldable trait를 구현하라.
import exercise.monoid.repeat01.Monoid._
