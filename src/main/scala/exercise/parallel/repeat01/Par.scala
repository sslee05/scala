package exercise.repeat01.parallel
...
object Par { self => 
  ...
  def parFilter[A](xs: List[A])(p: A => Boolean): Par[List[A]] = 
    map(sequence(xs.map(asyncF(a => if(p(a)) List(a) else Nil ))) )(x => x.flatten)
...
import exercise.repeat01.parallel.Par._
