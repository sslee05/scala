package exercise.parallel.repeat05
...
object Par { self =>
  ...
  def sortParViaMap(xs: Par[List[Int]]): Par[List[Int]] = 
    map(xs)(a => a.sorted)
  ...
  def parMap[A,B](xs: IndexedSeq[A])(f: A => B): Par[IndexedSeq[B]] = 
    sequenceBalance(xs.map(asyncF(f)(_)))
    
  //ex-14) 목록의 요소들을 병렬로 filter하는 parFilter를 구현하라.
  def parFilter[A](xs: List[A])(p: A => Boolean): Par[List[A]] = 
    map(sequence(xs.map(asyncF[A,List[A]]((a:A) => if(p(a)) List(a) else Nil))))(_.flatten)
  //def parFilter[A](xs: List[A])(p: A => Boolean): Par[List[A]] = ???
  ...
  def choiceViaFlatMap[A,V](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] = 
    flatMap(cond)(p => if(p) t else f)
  ...
  class ParOpt[A](pa: Par[A]) {
    def map2[B,C](pb: Par[B])(f: (A,B) => C): Par[C] = Par.map2(pa, pb)(f)
    def flatMap[B](f: A => Par[B]): Par[B] = Par.flatMap(pa)(f)
    def map[B](f: A => B): Par[B] = Par.map(pa)(f)
  }
  
}

import exercise.parallel.repeat05.Par._
...
object ParDriver extends App {
  ...
  val xs02 = List(unit(1),unit(2),unit(3))
  val rs02 = sequence(xs02)
  println(rs02)
