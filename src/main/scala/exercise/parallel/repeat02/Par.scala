package exercise.parallel.repeat02
...
object Par { self =>
  ...
  def fork[A](par: => Par[A]): Par[A] = es => {
    println(Thread.currentThread.getName +":"+Thread.currentThread.getId +": sub thread")
  ...
  def parFilter[A](xs: List[A])(p: A => Boolean): Par[List[A]] = {
    val rs = xs.map(asyncF[A,List[A]](a => if(p(a)) List[A](a) else Nil))
    map(sequence(rs))(a => a.flatten)
  }
...
import exercise.parallel.repeat02.Par._
...
object ParDriver extends App {
  ...
  //val xs = Vector(pa,pb,pb02,pb03)
  //val rs = sequenceBalance(xs)
  //val fc = rs(es)
  //println(fc.get)
  
  val xs = Vector(1,2,3,4)
  val rs = parMap(xs)(a => a + 1)(es)
  println(rs.get)
  
  println(choice(unit(true))(pa,pb)(es).get)
