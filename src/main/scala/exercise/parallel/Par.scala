...
object Par { self =>
  ...
  def map2[A,B,C](pa: Par[A], pb: Par[B])(f: (A,B) => C): Par[C] = es => {
    val fa = pa(es) // Par가 lazyUnit으로 생성시 비동기적으로 실행 unit으로 생성시 동기적으로 실행, Par가 unit으로 생성시 동기적으로 실행
    val fb = pb(es)
  ...
  def parMap[A,B](xs: IndexedSeq[A])(f: A => B): Par[IndexedSeq[B]] =  
    sequenceBalance(xs.map(asyncF(f))) // Par[IndexedSeq[B]](es) 할때 f함수의 처리를 병렬로 처리하게 된다.
    
  //ex-14) 목록의 요소들을 병렬로 filter하는 parFilter를 구현하라.
  def parFilter[A](xs: List[A])(p: A => Boolean): Par[List[A]] = {
    val rs:List[Par[List[A]]] =xs.map(asyncF[A,List[A]]((a:A) => if(p(a)) List(a) else Nil))
  ...
  def choice[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] = es =>
    if(run(es)(cond).get) t(es) else f(es) // 이는 cond의 실행결과까지 block된다.
