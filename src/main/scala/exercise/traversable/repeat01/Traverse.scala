package exercise.traversable.repeat01
...
trait Traverse[F[_]] extends Functor[F] with Foldable[F] { self =>
  ...
  val idMonad = new Monad[Id] {
    ...
    def flatMap[A,B](ma: Id[A])(f: A => B): Id[B] = f(ma)
  }
  ...
  override def foldMap[A,M](ma: F[A])(f: A => M)(mi: Monoid[M]): M = 
    traverse[({type f[x] = Const[M,x]})#f,A,Nothing](ma)(f)(mi)
  ...
  override def toList[A](ma: F[A]): List[A] =
    mapAccum(ma,Nil:List[A])((a,s) => (a,a::s))._2
...
object Traverse {
  ...
  def sequenceList[F[_]: Applicative,A](xs: List[F[A]])(implicit apt: Applicative[F]): F[List[A]] = 
    xs.foldRight[F[List[A]]](apt.unit(Nil))((a,b) => apt.map2(a, b)(_::_))
    
  //ex04) sequenceMap를 구현하라.
  def sequenceMap[F[_]: Applicative, K, V](xs: Map[K,F[V]])(implicit apt: Applicative[F]): F[Map[K,V]] =
    xs.foldLeft[F[Map[K,V]]](apt.unit(Map.empty)){
    case (b, (k,v)) => apt.map2(b, v)((b1,v1) => b1 + (k -> v1))
  }
    
  
  //ex-05) list용 Traverse instance 생성을 구현하라.
  val listTraverse = new Traverse[List] {
    def traverse[G[_], A, B](ma: List[A])(f: A => G[B])(implicit AG: Applicative[G]): G[List[B]] = 
      ma.foldRight[G[List[B]]](AG.unit(Nil))((a,b) => AG.map2(f(a), b)(_::_))
  }
  
  //ex-06) Option용 Traverse instance 생성을 구현하라.
  val optionTraverse = new Traverse[Option] {
    def traverse[G[_], A, B](ma: Option[A])(f: A => G[B])(implicit AG: Applicative[G]): G[Option[B]] = ma match {
  ...
  val treeTraverse = new Traverse[Tree] {
    def traverse[G[_], A, B](ma: Tree[A])(f: A => G[B])(implicit G: Applicative[G]): G[Tree[B]] = 
      G.map2(f(ma.head), listTraverse.traverse(ma.tail)(a => traverse(a)(f)) )(Tree(_,_)) 
  }
