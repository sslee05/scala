package exercise.applicative.repeat04
...
trait Applicative[F[_]] extends Functor[F] {
  ...
  def traverse[A,B](xs: List[A])(f: A => F[B]): F[List[B]] =
    xs.foldRight(unit(Nil:List[B]))((a,b) => map2(f(a),b)(_::_))
  ...
  def compose[G[_]](g: Applicative[G]): Applicative[({type f[x] = F[G[x]]})#f] = {
    ...
    new Applicative[({type f[x] = F[G[x]]})#f] {
      ...
      def map2[A,B,C](mga: F[G[A]], mgb: F[G[B]])(f: (A,B) => C): F[G[C]] = 
        self.map2(mga,mgb)((ga,gb) => g.map2(ga,gb)(f))
...
object ApplicativeDriver extends App {
  ...
  import exercise.applicative.repeat04.Applicative._
