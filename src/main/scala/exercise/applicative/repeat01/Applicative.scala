package exercise.applicative.repeat01
...
trait Applicative[F[_]] extends Functor[F] {
  ...
  def sequence[A](xs: List[F[A]]): F[List[A]] =
    xs.foldRight[F[List[A]]](unit(Nil))((a,b) => map2(a,b)((a1,b1) => a1::b1))
    
  //ex-05) map2 와 unit 으로 replicate 함수를 구현하라.
  def replicate[A](n: Int, ma: F[A]): F[List[A]] = 
    if(n > 0) map2(ma, replicate(n-1,ma))((a,b) => a :: b )
    else unit(Nil)
  ...
  //ex-09) map를 unit과 apply로 구현하라. Applicative 기본 Set02(unit,apply)
  ...
  def product[G[_]](G: Applicative[G]): Applicative[({type f[x] = (F[x],G[x])})#f] = {
    ...
    new Applicative[({type f[x] = (F[x],G[x])})#f] {
      ...
      override def apply[A,B](fab: (F[A => B],G[A => B]))(mab: (F[A], G[A])): (F[B],G[B]) = 
        (self.apply(fab._1)(mab._1), G.apply(fab._2)(mab._2) )
...
object Applicative {
  ...
  def validator[E]: Applicative[({type f[x] = Validation[E,x]})#f] =  {
...
object ApplicativeDriver extends App {
  ...
  import exercise.applicative.repeat01.Applicative._
