package exercise.applicative.repeat02
...
trait Applicative[F[_]] extends Functor[F] {
  ...
  def sequenceViaTraverse[A](ma: List[F[A]]): F[List[A]] = 
    traverse(ma)(a => a)
  ...
  def product[G[_]](G: Applicative[G]): Applicative[({type f[x] = (F[x],G[x])})#f] = {
    ...
    new Applicative[({type f[x] = (F[x],G[x])})#f] {
      ...
      override def map2ViaApply[A,B,C](ma: (F[A],G[A]), mb: (F[B],G[B]))(f: (A,B) => C): (F[C],G[C]) = 
        apply( (self.map(ma._1)(f.curried) , G.map(ma._2)(f.curried)) )(mb)
        
      override def apply[A,B](mf: (F[A => B], G[A => B]))(m: (F[A],G[A])): (F[B],G[B]) = 
        (self.apply(mf._1)(m._1), G.apply(mf._2)(m._2) )
      
      def map2ViaApply2[A,B,C](ma: (F[A],G[A]), mb: (F[B],G[B]))(f: (A,B) => C): (F[C],G[C]) = 
        (self.apply(self.apply(self.unit(f.curried))(ma._1))(mb._1), G.apply(G.apply(G.unit(f.curried))(ma._2))(mb._2) )
        
      def map2ViaApply3[A,B,C](ma: (F[A],G[A]), mb: (F[B], G[B]))(f: (A,B) => C): (F[C],G[C]) = 
        apply(apply(unit(f.curried))(ma))(mb)
...
object ApplicativeDriver extends App {
  ...
  val rs04 = opa.map2(Some(2), Some(3))(opa.productF(a => a * 2, b => b * 3)) ==
    opa.prodcut(opa.map(Some(2))(a => a * 2), opa.map(Some(3))(b => b * 3))
  println(rs04)
  
  
  import exercise.applicative.repeat02.Applicative._
