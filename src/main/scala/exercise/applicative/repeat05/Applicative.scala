package exercise.applicative.repeat05
...
trait Applicative[F[_]] extends Functor[F] {
  ...
  def traverse[A,B](ma: List[A])(f: A => F[B]): F[List[B]] = {
    type Z = F[List[B]] => F[List[B]]
    ma.foldLeft[Z]((y:F[List[B]]) => y)((b,a) => x => b(map2(f(a),x)(_::_)))(unit(Nil))
  }
  ...
  def sequence[A](xs: List[F[A]]): F[List[A]] = {
    type Z = F[List[A]] => F[List[A]]
    xs.foldLeft[Z]((y:F[List[A]]) => y)((b,a) => x => b(map2(a,x)(_::_)))(unit(Nil))
  }
    
  //ex-05) map2 와 unit 으로 replicate 함수를 구현하라.
  def replicate[A](n: Int, ma: F[A]): F[List[A]] = 
    if(n > 0) map2(ma,unit(n))((a,b) => List.fill(b)(a)) else unit(Nil)
  
  //ex-06) sequence를 이용하여 replicate를 구현하라.
  def replicateViaSequence[A](n: Int, ma: F[A]): F[List[A]] = 
    if(n > 0) sequence(List.fill(n)(ma)) else unit(Nil)
  ...
  def productF[I,O,I2,O2](f: I => O, g: I2 => O2): (I,I2) => (O,O2) = (i,i2) => 
    (f(i),g(i2))
  ...
  def compose[G[_]](G: Applicative[G]): Applicative[({ type f[x] = F[G[x]]})#f] = {
    ...
    new Applicative[({type f[x] = F[G[x]]})#f] {
      ...
      def map2[A,B,C](ma: F[G[A]], mb: F[G[B]])(f: (A,B) => C): F[G[C]] = 
        self.map2(ma, mb)((a,b) => G.map2(a, b)(f))
        
      override def apply[A,B](mf: F[G[A => B]])(ma: F[G[A]]): F[G[B]] = 
        self.map2(mf, ma)(G.apply(_)(_))
    }
  }
  ...
  def product[G[_]](G: Applicative[G]): Applicative[({type f[x] = (F[x],G[x])})#f] = {
    ...
    new Applicative[({type f[x] = (F[x],G[x])})#f] {
      ...
      override def apply[A,B](mf: (F[A => B], G[A => B]))(ma: (F[A], G[A])): (F[B], G[B]) = 
        (self.apply(mf._1)(ma._1), G.apply(mf._2)(ma._2))
        
      override def map2ViaApply[A,B,C](ma: (F[A],G[A]), mb: (F[B], G[B]))(f: (A,B) => C): (F[C],G[C]) = 
        (self.apply(self.apply(self.unit(f.curried))(ma._1))(mb._1), G.apply(G.apply(G.unit(f.curried))(ma._2))(mb._2))
    }
...
object Applicative {
  ...
  def eitherMonad[E]: basic.monad.Monad[({ type f[x] = Either[E,x]})#f] = new basic.monad.Monad[({ type f[x] = Either[E,x]})#f] {
...
object ApplicativeDriver extends App {
  ...
  import exercise.applicative.repeat05.Applicative._
