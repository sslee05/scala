package exercise.monad.repeat02
...
trait Monad[F[_]] extends Applicative[F] {
  ...
  def replicatedMViaSequence[A](n: Int, ma: F[A]): F[List[A]] = 
  ...
  def filterM[A](xs: List[A])(p: A => F[Boolean]): F[List[A]] = xs match {
      case Nil => unit(Nil)
      //case h::t => flatMap(p(h))(a => if(a) map2(unit(h),filterM(t)(p))((a1,b1) => a1::b1) else filterM(t)(p))
      case h:: t => flatMap(p(h))(a => if(a) map(filterM(t)(p))(b => h::b) else filterM(t)(p))
    }
...
object MonadDriver extends App {
  ...
  import exercise.monad.repeat02.Monad._
  ...
  val opManad = optionMonad
  println(opManad.replicate(3,Some("a")))
