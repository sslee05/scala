package exercise.monad.repeat05
...
trait Monad[F[_]] extends Applicative[F] {
  ...
  override def sequence[A](xs: List[F[A]]): F[List[A]] = 
   ...
   def _sequence[A](xs: List[F[A]]):F[List[A]] = {
    type B = F[List[A]] => F[List[A]]
    xs.foldLeft[B]((y:F[List[A]]) => y)((b,a) => x => b(map2(a,x)(_::_)))(unit(Nil))
  }
    
  //ex-13) traverse 를 구현하라.
  override def traverse[A,B](xs: List[A])(f: A => F[B]): F[List[B]] =
    xs.foldRight[F[List[B]]](unit(Nil))((a,b) => map2(f(a),b)(_::_))
    
  def _traverse[A,B](xs: List[A])(f: A => F[B]): F[List[B]] = {
    type X = F[List[B]] => F[List[B]]
    xs.foldLeft[X]((y:F[List[B]]) => y)((b,a) => x => b(map2(f(a),x)(_::_)))(unit(Nil))
  }
    
  //ex-14) replicate 를 구현하라.
  override def replicate[A](n: Int, ma: F[A]): F[List[A]] = 
    map2(ma,replicatedM(n-1,ma))(_::_)
  
  def replicatedM[A](n: Int, ma: F[A]): F[List[A]] = 
    if(n > 0) map(ma)(a => List.fill(n)(a))
    else unit(Nil)
  ...
  def _filterM[A](xs: List[A])(p: A => F[Boolean]): F[List[A]] = 
    xs.reverse.foldLeft(unit[List[A]](Nil))((b,a) => map2(p(a),b)((a1,b1) => if(a1) a::b1 else b1))
    
  def filterM[A](xs: List[A])(p: A => F[Boolean]): F[List[A]] = 
    map(sequence(xs.map(a => map(p(a))(a1 => if(a1) List(a) else Nil))))(_.flatten)
  ...
  def composeViaJoin[A,B,C](f: A => F[B], g: B => F[C]): A => F[C] = a =>
    join(map(f(a))(g))
...
object Reader {
 ...
 def readerMonad[R]: Monad[({type f[x] = Reader[R,x]})#f] = new Monad[({type f[x] = Reader[R,x]})#f] {
   ...
   def flatMap[A,B](ma: Reader[R,A])(f: A => Reader[R,B]): Reader[R,B] = Reader{ r =>
     val a = ma run r
     f(a) run r
   }
 }
...
object MonadDriver extends App {
  ...
  import exercise.monad.repeat05.Monad._
  ...
  val rxtmp = m3._sequence(List(s1,s2))
  println(rxtmp)
  
  val rx = List(1,2,3,4,5)
  val rxr = m3._filterM(rx)(a => if(a % 2 == 0) Some(false) else Some(true))
  println(rxr)
