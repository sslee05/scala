package exercise.monad.repeat03
...
trait Functor[F[_]] {
  ...
  def codistributep[A,B](e: Either[F[A],F[B]]): F[Either[A,B]] = e match {
...
trait Monad[F[_]] extends Applicative[F] {
  ...
  def filterM[A](xs: List[A])(p: A => F[Boolean]): F[List[A]] =
    xs.foldRight[F[List[A]]](unit(Nil))((a,b) => map2(p(a),b)((a1,b1) => if(a1) a::b1 else b1 ))
...
object Monad {
  ...
  val listMoand: Monad[List] = new Monad[List] {
  ...
  val optinMonad: Monad[Option] = new Monad[Option] {
...
object MonadDriver extends App {
  ...
  import exercise.monad.repeat03.Monad._
