package exercise.monad.repeat01
...
trait Functor[F[_]] {
  ...
  def distribute[A,B](e: Either[F[A],F[B]]): F[Either[A,B]] = e match {
    case Right(r) => map(r)(a => Right(a))
    case Left(l) => map(l)(a => Left(a))
  }
...
object Functor {
  ...
  //ex-04) 다음을 구현하라.
  def listFuntor: Functor[List] = new Functor[List] {
...
trait Monad[F[_]] extends Applicative[F] {
  ...
  def filterM[A](xs: List[A])(f: A => F[Boolean]): F[List[A]] = xs match {
      case Nil => unit(Nil)
      case h::t => map2(f(h), filterM(t)(f))((a,b) => if(a) h :: b else b ) 
  }
    
  def filterMByFlatmap[A](xs: List[A])(f: A => F[Boolean]): F[List[A]] = xs match {
    case Nil => unit(Nil)
    case h::t => flatMap(f(h))(a => if(a) map(filterMByFlatmap(t)(f))(b => h :: b) else filterMByFlatmap(t)(f))
  }
...
object MonadDriver extends App {
  ...
  import exercise.monad.repeat01.Monad._
  ...
  val filterMxs = List(1,2,3,4,5)
  val listM = listMonad
  val filterRx = listMonad.filterM(filterMxs)(x => listM.unit(x > 10))
  println(filterRx)
